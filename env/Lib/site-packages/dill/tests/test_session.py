...
def _error_line(error, obj, refimported):
    ...
    line = (
        traceback.format_exc().splitlines()[-2].replace("[obj]", "[" + repr(obj) + "]")
    )
...
def test_session_main(refimported):
    ...
    with TestNamespace(**extra_objects) as ns:
        try:
            ...
            error = sp.call(
                [python, __file__, "--child", str(refimported)], shell=shell
            )
            if error:
                sys.exit(error)
        finally:
...
def test_runtime_module():
    ...
    if mod is not runtime:
        print(
            "There are objects to save by referenece that shouldn't be:",
            mod.__dill_imported,
            mod.__dill_imported_as,
            mod.__dill_imported_top_level,
            file=sys.stderr,
        )
