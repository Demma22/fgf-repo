...
def test_mixins():
    ...
    dd = dill.detect
    assert (
        ds.getsource(dd.freevars(quadish)["f"])
        == "@quad_factory(a=0,b=4,c=0)\ndef quadish(x):\n  return x+1\n"
    )
    assert (
        ds.getsource(dd.freevars(quadruple)["f"])
        == "@doubler\ndef quadruple(x):\n  return 2*x\n"
    )
    assert ds.importable(quadish, source=False) == "from %s import quadish\n" % __name__
    assert (
        ds.importable(quadruple, source=False)
        == "from %s import quadruple\n" % __name__
    )
    assert (
        ds.importable(quadratic, source=False)
        == "from %s import quadratic\n" % __name__
    )
    assert (
        ds.importable(double_add, source=False)
        == "from %s import double_add\n" % __name__
    )
    assert (
