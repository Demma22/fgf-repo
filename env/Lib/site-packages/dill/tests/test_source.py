...
def test_importable():
    ...
    assert (
        getimportable(Foo.bar, alias="memo", byname=True)
        == "from %s import bar as memo\n" % __name__
    )
    assert (
        getimportable(Foo, alias="memo", byname=True)
        == "from %s import Foo as memo\n" % __name__
    )
    assert (
        getimportable(squared, alias="memo", byname=True)
        == "from %s import squared as memo\n" % __name__
    )
    assert (
        getimportable(squared, alias="memo", byname=False)
        == "memo = squared = lambda x:x**2\n"
    )
    assert (
        getimportable(add, alias="memo", byname=False)
        == "def add(x,y):\n  return x+y\n\nmemo = add\n"
    )
    ...
    assert (
        getimportable(squared, explicit=True) == "from %s import squared\n" % __name__
    )
...
def test_numpy():
    try:
        ...
        assert (
            getimportable(x, byname=False)
            == "from numpy import array\narray([1, 2, 3])\n"
        )
        assert (
            getimportable(array, byname=False)
            == "from %s import array\n" % array.__module__
        )
    except ImportError:
        pass
