...
def badobjects(obj, depth=0, exact=False, safe=False):
    ...
    return dict(
        (
            (attr, badobjects(getattr(obj, attr), depth - 1, exact, safe))
            for attr in dir(obj)
            if not pickles(getattr(obj, attr), exact, safe)
        )
    )
...
def badtypes(obj, depth=0, exact=False, safe=False):
    ...
    return dict(
        (
            (attr, badtypes(getattr(obj, attr), depth - 1, exact, safe))
            for attr in dir(obj)
            if not pickles(getattr(obj, attr), exact, safe)
        )
    )
...
def errors(obj, depth=0, exact=False, safe=False):
    ...
    if not depth:
        try:
            ...
            assert type(pik) == type(obj), "Unpickling produces %s instead of %s" % (
                type(pik),
                type(obj),
            )
            return None
